...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    def is_rate_limited(self, client_id: str) -> bool:
        ...
        self.minute_tracker[client_id] = [
            req_time for req_time in self.minute_tracker[client_id] 
            if now - req_time < 60
        ]
        self.second_tracker[client_id] = [
            req_time for req_time in self.second_tracker[client_id] 
            if now - req_time < 1
        ]
    ...
    def is_replay_attack(self, request: Request) -> bool:
        ...
        try:
            ...
            self.timestamp_cache = {
                req_id for req_id in self.timestamp_cache 
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if request.url.path.startswith("/api/v1/ingest/"):
            if self.is_replay_attack(request):
                ...
                return JSONResponse(
                    status_code=403,
                    content={"error": "Request timestamp invalid"}
                )
