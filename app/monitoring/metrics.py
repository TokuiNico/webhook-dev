...
active_subscriptions = Gauge(
    "active_subscriptions_total", "Number of active subscriptions", registry=registry
)
...
class MetricsCollector:
    ...
    def record_webhook_request(
        source: str, topic: str, status: str, duration: float | None = None
    ):
        """記錄 webhook 請求指標"""
        webhook_requests_total.labels(source=source, topic=topic, status=status).inc()
        if duration is not None:
            webhook_request_duration.labels(source=source, topic=topic).observe(
                duration
            )

    @staticmethod
    def record_webhook_dispatch(
        source: str, topic: str, status: str, duration: float | None = None
    ):
        """記錄 webhook 分發指標"""
        webhook_dispatch_total.labels(source=source, topic=topic, status=status).inc()
        if duration is not None:
            webhook_dispatch_duration.labels(source=source, topic=topic).observe(
                duration
            )
    ...
    def record_error(error_type: str, source: str = "unknown", topic: str = "unknown"):
        ...
        webhook_errors_total.labels(
            error_type=error_type, source=source, topic=topic
        ).inc()
